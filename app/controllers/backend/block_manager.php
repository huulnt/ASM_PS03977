...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (!empty($snapping_data['action'])) {
                    ...
                    if ($snapping_data['action'] == 'update' || $snapping_data['action'] == 'add') {
                        ...
                        if (is_numeric($result)) {
                            Tygh::$app['ajax']->assign('id', intval($result));
                        }
        ...
        if (!empty($_REQUEST['container_id']) && !empty($_REQUEST['linked_to_default'])) {
            ...
            if ($_REQUEST['linked_to_default'] == 'N') {
                ...
                if (empty($grids)) {
                    ...
                    $default_containers = Container::getList(array('default_location' => 1));
    ...
    if (defined('AJAX_REQUEST')) {
        if ($result === true) {
            Tygh::$app['ajax']->assign('status', 'OK');
        } elseif (is_numeric($result)) {
            Tygh::$app['ajax']->assign('id', intval($result));
            Tygh::$app['ajax']->assign('status', 'OK');
        } else {
            Tygh::$app['ajax']->assign('status', 'FAIL');
        }

        Tygh::$app['ajax']->assign('mode', $mode);
        Tygh::$app['ajax']->assign('action', $action);
...
if ($mode == 'manage' || $mode == 'manage_in_tab') {
    ...
    Tygh::$app['view']->assign('grid_params', $_REQUEST['grid_data']);
