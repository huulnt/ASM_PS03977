            ...
            if (fn_allowed_for('MULTIVENDOR')) {
                ...
                if (!empty($child_order_ids)) {
                    foreach ($child_order_ids as $child_order_id) {
                        ...
                        $status_data = fn_get_status_data($cart['payment_info']['order_status'], STATUSES_ORDER);
                        if (!empty($status_data['params']['remove_cc_info']) && $status_data['params']['remove_cc_info'] == 'Y') {
                            $cart['payment_info'] = fn_cleanup_payment_info('', $cart['payment_info'], true, true);
                        }
