...
{
    if (Registry::get('runtime.company_id') && in_array('product_name', $params['extend'])) {
...
function fn_ult_load_products_extra_data_post(&$products, $product_ids, $params, $lang_code)
{
    ...
    $extra_fields = array(
        '?:ult_product_descriptions' => array(
...
function fn_ult_db_query_executed(&$query, &$result)
{
    ...
    if (preg_match('/(?:INSERT|REPLACE)\s?INTO\s?([a-zA-Z_0-9]+)/i', $query, $tables)) {
        ...
        $object = str_replace(Registry::get('config.table_prefix'), '', $tables[2]);
...
function fn_ult_update_company_pre(&$company_data, &$company_id, &$lang_code, &$can_update)
{
    ...
    if ($company_id == 0 && $can_update) {
        $can_update = !Helpdesk::isCompaniesLimitReached();
        if (!$can_update) {
...
/**
 * Modifies locations used for the admin area and sets them the same values that are specified in the config.
 *
 ...
 */
function fn_ult_url_set_locations(&$url, &$area, &$protocol, &$lang_code, &$locations)
{
    $conf = Registry::get('config');
    $http_location  = empty($conf['origin_http_location'])?  $conf['http_location']  : $conf['origin_http_location'];
    $https_location = empty($conf['origin_https_location'])? $conf['https_location'] : $conf['origin_https_location'];

    $locations['A'] = array(
        'http'    => $http_location,
        'https'   => $https_location,
        'current' => defined('HTTPS') ? $https_location : $http_location,
        'rel'     => defined('HTTPS') ? $https_location : $http_location,
    );
}
