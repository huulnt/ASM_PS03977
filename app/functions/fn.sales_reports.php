...
function fn_get_max_value_report_interval($report, $table_id)
{
    ...
    if (!empty($report['tables'][$table_id])) {
        ...
        if ($element['code'] == 'order') {
            ...
            $add_field = db_quote(", ?:orders.b_county as `b_county`, ?:orders.b_state as `b_state`, ?:orders.s_county as `s_county`, ?:orders.s_state as `s_state`");
            $group_condition = " GROUP BY `interval`, `b_county`, `b_state`, `s_county`, `s_state`";
